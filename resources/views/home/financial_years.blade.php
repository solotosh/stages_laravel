...
<div class="container">
    <div class="header-section">
        <h1>In-Progress Stages for Financial Year: {{ $financialYear->name }}</h1>
        <p>{{ $financialYear->description }}</p>
    </div>
        ...
        @foreach($stages as $stage)
            <div class="timeline-item">
                ...
                <div class="content">
                    ...
                    <div class="details">
                        <div class="detail-item"><strong>Sub County:</strong> {{ $stage->subcounty->sub_county_name ?? 'N/A' }}</div>
                        <div class="detail-item"><strong>Ward:</strong> {{ $stage->ward->ward_name ?? 'N/A' }}</div>
                        <div class="detail-item"><strong>Budget:</strong> KES {{ number_format($stage->amount, 2) }}</div>
                        <div class="detail-item"><strong>Financial Year:</strong> {{ $stage->financialYear->name ?? 'N/A' }}</div>
                    </div>
    ...
    <div class="modal-content">
        ...
        <form id="modalForm" method="POST" action="{{ route('feedbackstore') }}">
            ...
            <div class="form-group">
                ...
                <input type="tel" id="modalPhone" name="phone" class="form-control" required>
            </div>
            <div class="form-group">
                <label for="modalText">Comment</label>
                <textarea id="modalText" name="comment" class="form-control" rows="4" required></textarea>
            </div>
...
<script>
    function openModal(type, stageId) {
        ...
        modalTitle.textContent = type === 'feedback' ? 'Add Feedback' : 'Add Remark';
        modalForm.action = type === 'feedback' ? "{{ route('feedbackstore') }}" : "{{ route('remarks.store') }}";
